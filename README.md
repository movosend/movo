...
- `https://localhost/api/v1/...` → gateway → microservicio correspondiente, según el mapa declarativo de `gateway/src/config/routes-map.ts`. Todo bajo `/api/v1` requiere `Authorization: Bearer <jwt>` **excepto** los endpoints explícitamente públicos: `POST /api/v1/auth/register`, `/login`, `/refresh`, `/send-otp`, `/verify-otp`, `/resend-otp` (MOVO-71 — reemplaza el `/verify-phone` de MOVO-68, ya sin uso), `POST /api/v1/kyc/session`, `GET /api/v1/kyc/status` y `POST /api/v1/kyc/webhook` (MOVO-72). El gateway devuelve 401 si falta el token en cualquier otra ruta.
