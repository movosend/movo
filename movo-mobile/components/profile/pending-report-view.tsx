 ...
 * y cuándo. Vive dentro del scroll de la pantalla de reporte, sin scroll propio: el
...
export function PendingReportView({
  ...
  const canSubmit = details.trim().length > 0 && !addEntryMutation.isPending;
  ...
  return (
    <View testID={testID}>
      ...
      {notice ? (
        <View className="mb-4 rounded-lg bg-bg-mute px-3.5 py-3" testID={`${testID}-notice`}>
          <Text className="font-sans text-small text-fg-2">{notice}</Text>
        </View>
      ) : null}

      <Text className="mb-2 font-sans-semibold text-caption uppercase text-fg-3">Lo que enviaste</Text>
      <View className="mb-6 gap-2.5" testID={`${testID}-history`}>
