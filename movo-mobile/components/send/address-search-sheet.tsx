import { ChevronLeft, MapPin, Navigation, Search, Star } from "lucide-react-native";
...
import { placesClient, type PlacePrediction } from "../../src/api/places-client";
...
export function AddressSearchSheet({
  ...
  useEffect(() => {
    ...
    placesClient
      ...
      .catch(() => {
        if (!cancelled) {
          setPredictions([]);
          setError("No pudimos buscar direcciones ahora. Probá de nuevo en un momento.");
        }
      })
      .finally(() => {
  ...
  const handleSelectPrediction = async (prediction: PlacePrediction) => {
    ...
    try {
      ...
      onSelect({ address: details.formattedAddress, lat: details.lat, lng: details.long, source: "places" });
    } catch {
  ...
  const handleSelectSaved = (saved: Address) => {
    onSelect({
      address: saved.label ?? `${saved.street} ${saved.streetNumber}, ${saved.city}`,
      lat: saved.lat,
      lng: saved.long,
      source: "saved",
    });
  ...
  return (
    <Modal visible={visible} animationType="slide" onRequestClose={onClose} testID={testID}>
      <SafeAreaView className="flex-1 bg-bg" edges={["top", "bottom"]}>
        ...
        <View className="flex-row items-center gap-2.5 rounded-lg border border-border-strong px-3.5 py-2.5 mx-4 mb-3">
          ...
          />
          {loading || resolving ? <ActivityIndicator size="small" color={colors.fg3} /> : null}
        </View>
        ...
        <ScrollView className="flex-1 px-4" keyboardShouldPersistTaps="handled">
          ...
          >
            ...
            <Text className="font-sans-medium text-[14px] text-fg">Usar mi ubicación actual</Text>
          </Pressable>
          ...
          {!showResults && savedAddresses && savedAddresses.length > 0 ? (
            <View testID={testID ? `${testID}-saved` : undefined}>
              ...
              {savedAddresses.map((saved) => (
                <Pressable
                  key={saved.id}
                  testID={testID ? `${testID}-saved-${saved.id}` : undefined}
                ...
                >
                  ...
                  <View className="flex-1">
                    <Text className="font-sans-medium text-[14px] text-fg" numberOfLines={1}>
            ...
            <View testID={testID ? `${testID}-results` : undefined}>
              {predictions.map((prediction) => (
                <Pressable
                  ...
                  testID={testID ? `${testID}-result-${prediction.placeId}` : undefined}
          ...
          {error ? <Text className="mt-3 font-sans text-[12px] text-danger-500">{error}</Text> : null}
        </ScrollView>
      </SafeAreaView>
    </Modal>
  );
