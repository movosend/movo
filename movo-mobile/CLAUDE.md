...
- **Copy de precio en vista emisor y precio acordado**: se reemplazó "Precio sugerido" por "Costo aproximado" en `components/send/price-preview-card.tsx`. En Detalle de Envío (`app/(app)/shipments/[id].tsx`), al aceptar una oferta se exhibe el precio pactado con la etiqueta "Precio acordado" (precio final/confirmado, no aproximado), resolviendo la omisión de persistencia de `agreedPriceArs` en `offer-repository.ts#acceptOffer` e integrando fallback defensivo en `getShipmentDetail`.
