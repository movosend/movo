...
- **Centrado de cámara, sincronización bidireccional y deep link robusto en mapa**: `RouteMap` deriva los desplazamientos verticales de centrado dinámicamente a partir de `bottomOffset` y gestiona el rechazo de `Linking.openURL` con feedback toast de error ("No se pudo abrir la navegación externa."). Código muerto `EDGE_PADDING` removido.
- **Sincronización bidireccional y selección visual (AC4)**: `isSelected` aplica resaltado visual explícito tanto en el marcador del mapa (crece de 28px a 34px con borde `#C6F24A` verde lima y sombra) como en la fila de la lista de paradas (`StopList` con borde `#C6F24A`, fondo sutil y `accessibilityState.selected`).
...
- **Prioridad de destino en 'Abrir en Maps'**: `handleOpenExternalMaps` prioriza la parada seleccionada en el mapa (`selectedStopOrder`) si el usuario interactuó con un pin específico, cayendo a la parada activa (`activeStopOrder ?? 1`) por defecto.
...
Tests: `test/route-screen.test.tsx` (montaje, estados de carga, vacío, sin GPS, error, demo, navegación a wizard y refresco manual), `test/route-components.test.tsx` (unitarios de `StopList` y `RouteMap`, AC2-AC6, AC9, AC11, highlight de selección, prioridad de mapa externo, controles, drag handle y error en deep link), `test/use-optimized-route.test.ts` (hook, focus effect, guard de secuencia contra race conditions, limpieza de ruta en refetch fallido, GPS y errores), y `test/transport-screen.test.tsx` (acceso a /route). 29 tests específicos pasando en verde. `tsc --noEmit` sin errores.
